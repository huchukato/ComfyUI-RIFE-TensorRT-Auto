# https://github.com/Fannovel16/ComfyUI-Frame-Interpolation/tree/main/vfi_models/rife
...
def conv(
    ...
    if arch_ver in ["4.2", "4.3", "4.5", "4.6", "4.7", "4.10", "4.17", "4.26", "4.25"]:
        return nn.Sequential(
            nn.Conv2d(
                in_planes,
                out_planes,
...
class IFBlock(nn.Module):
    def __init__(self, in_planes, c=64, arch_ver="4.0"):
        ...
        if arch_ver in ["4.6", "4.7", "4.10", "4.17"]:
         ...
         # add 4.26/4.25 support
        if arch_ver in ["4.26", "4.25"]:
            self.convblock = nn.Sequential(
                ResConv(c),
                ResConv(c),
                ResConv(c),
    ...
    def forward(self, x, flow=None, scale=1):
        ...
        if self.arch_ver in ["4.2", "4.3", "4.5", "4.6", "4.7", "4.10", "4.17", "4.26", "4.25"]:
...
"""
currently supports 4.0-4.12, 4.17, 4.26
...
class IFNet(nn.Module):
    def __init__(self, arch_ver="4.0"):
        ...
        if arch_ver in ["4.26"]:
            self.block0 = IFBlock(7+16, c=192, arch_ver=arch_ver)
            self.block1 = IFBlock(8+4+16+8, c=128, arch_ver=arch_ver)
            self.block2 = IFBlock(8+4+16+8, c=96, arch_ver=arch_ver)
            self.block3 = IFBlock(8+4+16+8, c=64, arch_ver=arch_ver)
            self.block4 = IFBlock(8+4+16+8, c=64, arch_ver=arch_ver)
            self.encode = Head_417()
        # add 4.25 support (same structure as 4.26 but smaller encode head and block4)
        if arch_ver in ["4.25"]:
            self.block0 = IFBlock(7+8, c=192, arch_ver=arch_ver)
    ...
    def forward(
        ...
        if self.arch_ver in ["4.7", "4.10", "4.17", "4.26", "4.25"]:
